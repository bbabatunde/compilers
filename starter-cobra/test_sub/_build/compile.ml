...
let const_bool_tag = HexConst(0x00000001);;
...
let rec compile_expr (e : tag expr) (si : int) (env : (string * int) list) : instruction list =
  ...
  | EPrim2(op,left,right,tag)-> 
      let instr =
        (compile_expr left (si) env) @
        [IMov(RegOffset(~-si, EBP), Reg(EAX))] @
        (compile_expr right (si + 1) env) @
  ...
  | EIf(pred, body, els, tag) -> 
   let true_label = sprintf "if_true_%s" (string_of_int tag) in
      ...
      (compile_expr pred si env) @
      [ICmp(Reg(EAX), const_true)] @
      [IJne(false_label)] @
      [ILabel(true_label)] @
      (compile_expr body (si + 1) env) @
      [IJmp(done_label)] @
      [ILabel(false_label)] @
      [ICmp(Reg(EAX), const_false)] @
      [IJne("error_not_boolean_if")] @
      (compile_expr els (si + 2) env) @
      [ILabel(done_label)]
...
our_code_starts_here:" in
  let count = word_size *  count_vars anfed in
  let stack_setup = [
      IPush(Reg(EBP));
