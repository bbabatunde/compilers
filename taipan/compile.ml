    ...
    let e_reg = compile_imm e env in
      [IMov(Reg(EAX), e_reg)] @
          (match op with
           ...
           | IsBool -> [IAnd(Reg(EAX), HexConst(0x1)); IShl(Reg(EAX), HexConst(31)); IXor(Reg(EAX), const_false)]

           | IsNum  -> [IAnd(Reg(EAX), HexConst(0x1)); IShl(Reg(EAX), HexConst(31)); IXor(Reg(EAX), const_true)]
  ...
  | CPrim2(op, left, right, tag) ->
    let left_val = compile_imm left env in
      let right_val = compile_imm right env in
        let end_label = cmp_label op tag in 

        let prefix, suffix = (typechecks left_val right_val op) in
        let body =
            (match op with
             | And  ->  [ IMov(Reg(EAX), left_val);    
                          IAnd(Reg(EAX), right_val)]   

             | Or  ->  [ IMov(Reg(EAX), left_val);   
                          IOr(Reg(EAX), right_val)]   
   ...
   | CApp(name, exprs, _) ->
       ...
       if is_tail=true && num_args=(List.length exprs) then
         ...
         @ [ IAdd(Reg(ESP), HexConst(4*(List.length exprs))) ] 
         )
